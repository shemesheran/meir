...
class MeirPodcastRSSGenerator:
    ...
    def generate_lessons_podcast_rss(self, lessons_set):
        feed_generator = self.init_feed_generator()
        for lesson in lessons_set:
            feed_entry = feed_generator.add_entry()
            self.edit_podcast_feed_entry(feed_entry, lesson)

        return feed_generator.rss_str()

    def init_feed_generator(self):
        feed_generator = FeedGenerator()
        feed_generator.load_extension('podcast')
        feed_generator.title(self.title)
        feed_generator.link(self.generate_podcast_link(self.link))
    ...
    def generate_podcast_link(self, link):
        return {"href": link}

    def edit_podcast_feed_entry(self, feed_entry, lesson):
        feed_entry.id(str(lesson.code))
        feed_entry.link(self.generate_podcast_link(lesson.audio_url))
